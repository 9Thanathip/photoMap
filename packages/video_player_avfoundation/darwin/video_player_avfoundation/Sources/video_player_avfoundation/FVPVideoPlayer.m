...
/// Returns YES if the track is HDR (HLG or PQ transfer function). iPhone
/// records HDR video in one of these two; everything else is SDR.
static BOOL FVPVideoTrackIsHDR(AVAssetTrack *videoTrack) {
        ...
        if ([tracks count] > 0) {
          ...
          void (^trackCompletionHandler)(void) = ^{
            ...
            if ([videoTrack statusOfValueForKey:@"preferredTransform"
              ...
              BOOL needsTransform =
                  !CGAffineTransformIsIdentity(self->_preferredTransform);
              BOOL isHDR = FVPVideoTrackIsHDR(videoTrack);
              // Do not use video composition when it is not needed.
              if (!needsTransform && !isHDR) {
                return;
              }
              // Note:
              ...
              if (isHDR) {
                // HDR (HLG/PQ) blows out under the raw AVPlayerLayer
                // pass-through in Flutter's SDR compositing window. Force a
                // color-managed HDR->SDR tone-map to Rec.709 — the same
                // high-quality path Apple Photos uses, without the naive
                // 8-bit color shift of the texture output.
